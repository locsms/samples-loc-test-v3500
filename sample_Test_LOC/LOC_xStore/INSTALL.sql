...
@fmt(CMP,@dbHot(INI,@runSmsCli.prop,,Install.sample.switch.devops)=1,'®wizRpl(DevOps=-devops)');
...
@EXEC(EXE='@OfficeHtm\dev\xStore\build-production@fmt(CMP,@dbHot(FindFirst,@OfficeHtm\dev\xStore\build-production@wizGet(DevOps).bat)=,,@wizGet(DevOps)).bat @Office');
